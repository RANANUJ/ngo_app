...
import 'package:ngo_app/shared/widgets/emergency/full_screen_sos_alert.dart';
...
Future<void> showNativeSOSAlert(Map<String, dynamic> data) async {
  try {
    ...
    secureLog('Error showing native SOS alert: $e');
  }
}
...
class NotificationService {
  ...
  Future<void> initialize() async {
    ...
    secureLog('FCM Token: $_fcmToken');

    // Listen to token refresh
    _messaging.onTokenRefresh.listen((newToken) {
      _fcmToken = newToken;
      secureLog('FCM Token refreshed: $newToken');
    });
  ...
  Future<void> _requestPermission() async {
    ...
    secureLog('Notification permission: ${settings.authorizationStatus}');
  }
  ...
  Future<void> _initializeLocalNotifications() async {
    ...
    await _localNotifications.initialize(
      ...
      onDidReceiveNotificationResponse: (NotificationResponse response) {
        secureLog('Notification tapped: ${response.payload}');
  ...
  void _handleForegroundMessage(RemoteMessage message) {
    secureLog('Foreground message received: ${message.notification?.title}');
    secureLog('Message data: ${message.data}');
    
    final data = message.data;
    final type = data['type']?.toString().toLowerCase() ?? '';
    
    secureLog('Notification type: $type');
    
    // Check if this is an SOS alert
    if (type == 'sos_alert') {
      secureLog('🚨 SOS ALERT DETECTED - Triggering emergency sound and vibration!');
  ...
  void _showFullScreenSOSAlert(Map<String, dynamic> data) {
    ...
    if (context == null) {
      secureLog('Navigator context not available for full-screen alert');
      return;
    }
    ...
    FullScreenSOSAlert.show(
      ...
      onViewDetails: () {
        secureLog('View details tapped for SOS: ${data['sosId']}');
        // Navigate to SOS details - handled by the caller
      },
      onDismiss: () {
        secureLog('SOS alert dismissed');
      },
    );
  }

  void _handleNotificationTap(RemoteMessage message) {
    secureLog('Notification tapped: ${message.data}');
  ...
  void _handleLocalNotificationTap(String? payload) {
    if (payload != null) {
      try {
        ...
        secureLog('Local notification payload: $data');
      } catch (e) {
        secureLog('Error parsing notification payload: $e');
      }
    }
  }
  ...
  Future<void> _playEmergencyAlertSound() async {
    secureLog('🔊 _playEmergencyAlertSound called');
    
    if (_isPlayingEmergencySound) {
      secureLog('⚠️ Emergency sound already playing, skipping');
      return;
    }
    
    try {
      _isPlayingEmergencySound = true;
      secureLog('✅ Starting emergency alert sound and vibration');
      ...
      try {
        ...
        secureLog('🎵 Attempting to play emergency_alert.mp3');
        await _emergencyPlayer.play(AssetSource('emergency_alert.mp3'));
        secureLog('✅ Emergency sound playing');
      } catch (e) {
        secureLog('❌ Custom sound not found, using system sound: $e');
