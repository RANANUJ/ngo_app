...
import 'package:ngo_app/features/donations/domain/models/subscription.dart';
import 'package:ngo_app/features/donations/domain/models/donation_stats.dart';
...
class _NgoMonthlyFundingScreenState extends State<NgoMonthlyFundingScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    await donationController.loadNgoFundingDashboardData(
      ngoId: widget.ngoId,
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading = donationController.isLoading;
    ...
    return Scaffold(
      ...
      body: isLoading 
          ...
          : TabBarView(
              controller: _tabController,
              children: [
                _buildOverviewTab(donationController),
                _buildTransactionsTab(donationController),
                _buildSubscriptionsTab(donationController),
                _buildDonorsTab(donationController)
              ],
            ),
    );
  }

  Widget _buildOverviewTab(DonationController controller) {
  ...
  Widget _buildStatCards(DonationController controller) {
    final stats = controller.monthlyStats;
    return GridView.count(
      crossAxisCount: 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
      mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.4,
      children: [
        _statCard('Total Raised', currencyFormat.format(stats?.totalAmount ?? 0), Icons.currency_rupee, Colors.green, () => _showDetailDialog('Total Raised', 'Total amount raised this month from all monthly donations.', currencyFormat.format(stats?.totalAmount ?? 0))),
        _statCard('Transactions', '${stats?.totalTransactions ?? 0}', Icons.receipt_long, Colors.blue, () { _tabController.animateTo(1); }),
        _statCard('Unique Donors', '${stats?.totalDonors ?? 0}', Icons.people, Colors.orange, () { _tabController.animateTo(3); }),
        _statCard('Avg. Donation', currencyFormat.format(stats?.averageDonation ?? 0), Icons.analytics, Colors.purple, () => _showDetailDialog('Average Donation', 'Average donation amount per transaction this month.', currencyFormat.format(stats?.averageDonation ?? 0))),
        _statCard('Active Plans', '${stats?.activeSubscriptions ?? 0}', Icons.subscriptions, Colors.teal, () { _tabController.animateTo(2); }),
        _statCard('Pending Thanks', '${stats?.thankYouPending ?? 0}', Icons.mail_outline, Colors.red, () => _showPendingThanks(controller)),
      ],
    );
  }
  ...
  void _showPendingThanks(DonationController controller) {
    final pending = controller.monthlyTransactions.where((t) => !(t.thankYouSent) && t.status == 'completed').toList();
  ...
  Widget _buildTrendChart(DonationController controller) {
    final trend = controller.yearlyTrend;
    if (trend.isEmpty) return const SizedBox.shrink();
    final maxValue = trend.values.fold<double>(1, (a, b) => a > b ? a : b);
    return Container(
      padding: const EdgeInsets.all(16),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 16),
        SizedBox(height: 150, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: trend.entries.map((e) {
  ...
  Widget _buildCategoryBreakdown(DonationController controller) {
    final stats = controller.monthlyStats;
    if (stats?.categoryWiseAmount.isEmpty ?? true) return const SizedBox.shrink();
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...stats!.categoryWiseAmount.entries.map((e) {
          final percentage = (stats.totalAmount > 0) ? (e.value / stats.totalAmount * 100) : 0;
          return GestureDetector(
            ...
            child: Padding(padding: const EdgeInsets.only(bottom: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              LinearProgressIndicator(value: percentage / 100, backgroundColor: Colors.grey.shade200, valueColor: const AlwaysStoppedAnimation(primary)),
            ])),
          );
        }).toList(),
      ]),
  ...
  Widget _buildTransactionsTab(DonationController controller) {
    final transactions = controller.monthlyTransactions;
    ...
    final groupedTransactions = <String, List<Donation>>{};
    for (final t in transactions) {
      final monthKey = DateFormat('MMMM yyyy').format(t.createdAt);
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final monthList = groupedTransactions[month]!;
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...monthList.map((t) => _buildTransactionTile(t)),
        ]);
      },
    );
  }

  Widget _buildTransactionTile(Donation donation) {
    return Container(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          backgroundImage: donation.profileImageUrl != null ? NetworkImage(donation.profileImageUrl!) : null,
          child: donation.profileImageUrl == null ? Text(donation.donorName.isNotEmpty ? donation.donorName[0].toUpperCase() : 'A', style: TextStyle(color: primary, fontWeight: FontWeight.bold)) : null,
        ),
        ...
        subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(DateFormat('dd MMM yyyy, hh:mm a').format(donation.createdAt), style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
        ...
        trailing: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(donation.status.toUpperCase(), style: TextStyle(fontSize: 9, color: donation.status == 'completed' || donation.status == 'success' ? Colors.green : Colors.orange)),
          ]),
        ]),
  ...
  void _showTransactionDetails(Donation donation) {
    showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            CircleAvatar(radius: 30, backgroundColor: primary.withValues(alpha: 0.1),
              backgroundImage: donation.profileImageUrl != null ? NetworkImage(donation.profileImageUrl!) : null,
              child: donation.profileImageUrl == null ? Text(donation.donorName.isNotEmpty ? donation.donorName[0].toUpperCase() : 'A', style: TextStyle(color: primary, fontWeight: FontWeight.bold, fontSize: 24)) : null),
          ...
          _detailRow(Icons.calendar_today, 'Date', DateFormat('dd MMMM yyyy, hh:mm a').format(donation.createdAt), Colors.blue),
          ...
          _detailRow(Icons.check_circle, 'Status', donation.status.toUpperCase(), donation.status == 'completed' || donation.status == 'success' ? Colors.green : Colors.orange),
          ...
          if (!donation.thankYouSent && donation.donorEmail.isNotEmpty)
            SizedBox(width: double.infinity, child: ElevatedButton.icon(
              ...
              icon: const Icon(Icons.mail, color: Colors.white), label: const Text('Send Thank You Email', style: TextStyle(color: Colors.white)),
  ...
  Widget _buildSubscriptionsTab(DonationController controller) {
    final subscriptionsList = controller.subscriptions;
    ...
    final active = subscriptionsList.where((s) => s.status == 'active').toList();
    final paused = subscriptionsList.where((s) => s.status == 'paused').toList();
    final other = subscriptionsList.where((s) => s.status != 'active' && s.status != 'paused').toList();
  ...
  Widget _sectionHeader(String title, IconData icon, Color color, int count) {
    return Padding(padding: const EdgeInsets.only(bottom: 12), child: Row(children: [
      ...
      const Spacer(),
      Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: color.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
  ...
  Widget _buildSubscriptionTile(Subscription sub) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: sub.status == 'active' ? Colors.green.withValues(alpha: 0.3) : Colors.orange.withValues(alpha: 0.3))),
      child: ListTile(
        contentPadding: const EdgeInsets.all(12),
        leading: CircleAvatar(backgroundColor: primary.withValues(alpha: 0.1),
        ...
        trailing: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: sub.status == 'active' ? Colors.green.withValues(alpha: 0.1) : Colors.orange.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)),
  ...
  Widget _buildDonorsTab(DonationController controller) {
    final topDonorsList = controller.topDonors;
    ...
    return ListView.builder(padding: const EdgeInsets.all(16), itemCount: topDonorsList.length, itemBuilder: (context, index) {
      final donor = topDonorsList[index];
      return Container(
        ...
        child: InkWell(
          ...
          child: Row(children: [
            Stack(children: [
              CircleAvatar(radius: 24, backgroundColor: primary.withValues(alpha: 0.1),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                const SizedBox(width: 4),
                Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: primary.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
  ...
  void _showDonorDetails(DonorSummary donor) {
    showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (context) => Container(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Stack(children: [
            CircleAvatar(radius: 35, backgroundColor: primary.withValues(alpha: 0.1),
              backgroundImage: donor.donorProfileImage != null ? NetworkImage(donor.donorProfileImage!) : null,
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (donor.isTopDonor) Container(margin: const EdgeInsets.only(top: 4), padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.amber.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(8)),
              child: const Text('⭐ Top Donor', style: TextStyle(fontSize: 11, color: Colors.amber, fontWeight: FontWeight.bold))),
          ])),
        ]),
        const SizedBox(height: 24),
        Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
        ...
        if (donor.donorEmail.isNotEmpty)
          SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: () { Navigator.pop(context); _sendThankYouToDonor(donor); },
            icon: const Icon(Icons.mail, color: Colors.white), label: const Text('Send Thank You Email', style: TextStyle(color: Colors.white)), style: ElevatedButton.styleFrom(backgroundColor: primary, padding: const EdgeInsets.all(14)))),
      ])),
    );
  }
  ...
  Future<void> _sendThankYou(Donation donation) async {
    final donationController = context.read<DonationController>();
    final ngoEmail = widget.ngoEmail ?? 'noreply@ngoapp.com';
    
    final success = await donationController.sendThankYouEmail(
  ...
  Future<void> _sendThankYouToDonor(DonorSummary donor) async {
    ...
    final success = await donationController.sendThankYouEmail(
      donationId: '', // Dummy id for donor summary level thanks
      donorEmail: donor.donorEmail,
      donorName: donor.donorName,
      amount: donor.totalAmount,
      campaignTitle: 'Monthly Donations',
      ngoName: widget.ngoName,
      donationDate: donor.lastDonation,
      ngoEmail: ngoEmail,
    );
