...
class _EditOpportunityScreenState extends State<EditOpportunityScreen> {
  ...
  void initState() {
    ...
    if (eventDateData != null) {
      if (eventDateData is DateTime) {
        _eventDate = eventDateData;
      } else {
        try {
          _eventDate = (eventDateData as dynamic).toDate();
        } catch (_) {}
      }
    }
  }
  ...
  Future<List<String>> _uploadNewImages() async {
    ...
    final ngoId = widget.opportunityData['ngoId'] ?? 'unknown_ngo';
  ...
  Future<void> _updateOpportunity() async {
    ...
    try {
      ...
      final opportunity = Opportunity(
        ...
        ngoId: widget.opportunityData['ngoId'] ?? '',
        ngoName: widget.opportunityData['ngoName'] ?? '',
        ...
        applicationsCount: widget.opportunityData['applicationsCount'] ?? 0,
        status: widget.opportunityData['status'] ?? 'active',
        createdAt: widget.opportunityData['createdAt'] is DateTime
            ? widget.opportunityData['createdAt']
            : null,
      );
